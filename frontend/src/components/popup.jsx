...
import { GetIntaSendUrl } from "../fetch/fetch";
      ...
      <div className="relative w-full max-w-[600px] p-4">
        <section className="bg-white rounded-lg shadow-lg dark:bg-gray-800">
          ...
          <div className="p-6 flex justify-center items-center flex-col">
            ...
            {clickCryptoMethod&&(
              <>
              <input type="text" className="w-full px-4 py-2 rounded bg-white text-black" onChange={(e)=>setInputWalletAddress(e.target.value)} value={inputWalletAddress} placeholder="Enter your wallet address" />
              <div className="w-full flex items-center justify-between gap-4 p-2 rounded-lg shadow-md">
  <select className="w-[50%] p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" name="currency" value={currency} onChange={(e)=>setCurrency(e.target.value)}>
    <option value="BTC">BTC</option>
    <option value="ETH">ETH</option>
  </select>
  
  <button  onClick={()=>handleCryptoDeposit()} className=" bg-yellow-600 text-white py-2 px-4 rounded-md hover:bg-yellow-700 transition duration-300">
    Send Wallet Address
  </button>
</div>
</>      )}
            {clickCryptoMethod&& walletAddress && (
                <div className="mt-5">
                  ...
                  <p className="text-red-500 mt-3">Kindly send {amountIncrypto +" "+currency } to the above address using your wallet address.</p>
